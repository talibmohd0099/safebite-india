...
//
// The rule (clarified after an earlier over-correction this session):
// keep our own copy of every Blinkit product photo, useful both for
// identifying the product and as a reference for sourcing a real/raw
// photo of it later -- what's not wanted is a report's own imageUrl
// ever pointing a user's browser at Blinkit's server directly. A
// report is a frozen snapshot from generation time, so optimizing the
// source row LATER never reaches back into an already-saved report --
// this copies blinkit_products.optimized_image_url (OUR OWN Storage
// copy, never image_url/Blinkit's own server) into report.imageUrl
// wherever the two disagree. No downloading, no re-processing -- purely
// a data sync, so it's fast and safe to re-run.
//
// Usage:
...
// The background scrape loop hammers the same tables with writes while
// this reads -- a real "canceling statement due to statement timeout"
// was seen live under that load. Retries, not a bigger timeout: the
// transient contention passes within a few seconds either way, and a
// smaller page size means less work wasted per hit.
    ...
    for (const row of data) {
      ...
      if (write) {
        const { error: updateError } = await withRetry(() =>
          supabase.from('product_reports').update({ report: { ...row.report, imageUrl: optimizedUrl } }).eq('id', row.id),
        );
